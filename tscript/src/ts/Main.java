...
public class Main
{
	...
	private static void GenerateFunctionClass(Encode.ReturnValue funcinfo){
		...
		CtClass theClass = pool.makeClass(funcinfo.result);

		// class will implement support.TSCode interface
		try {
			theClass.addInterface(pool.get(TSCode.class.getName()));
		} catch (NotFoundException ex) {
			Message.fatal("cannot add TSCode interface to "+funcinfo.result+" class");
		}
		...
		try {
			...
			Message.fatal("cannot add a default constructor to the "+funcinfo.result+" class: " +
					ex.getReason());
		}

		// now make a execute method
		String theMethodBody =funcinfo.code;

		CtMethod theMethod = null;
		try {
			theMethod = CtNewMethod.make(theMethodBody, theClass);
		} catch (CannotCompileException ex) {
			Message.fatal("cannot compile the generated "+funcinfo.result+" method body: " +
					ex.getReason());
		}
		...
		try {
			...
			Message.fatal("cannot add the execute method into the "+funcinfo.result+" class: " +
					ex.getReason());
		}
	}
	...
	public static void main(String[] args) throws IOException
	{
		...
		for (Object item : root)
		{
			...
			if (t instanceof FunctionExpression)
				Message.log("here is a FunctionExpression");
		...
		if (doDumpJava)
		{
			...
			//I only output the method block for debug 
			for (Encode.ReturnValue funcinfo: genCode.getFuncRec()){
				PrintWriter method = openOutputFile(funcinfo.result, ".execute");
				method.println(funcinfo.code);
				method.close();
			}
		}
		...
		if (doExec)
		{
			...
			for (Encode.ReturnValue funcinfo: genCode.getFuncRec())
				GenerateFunctionClass(funcinfo);
